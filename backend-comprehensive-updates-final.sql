-- ===============================================
-- School Platform - Comprehensive Backend Updates (FINAL FIX)
-- Run this entire script in Supabase SQL Editor
-- Drops ALL possible dependent views first
-- ===============================================

-- 1. Drop ALL existing views that might depend on student_profiles columns
-- (Using CASCADE to ensure all dependencies are removed)
DROP VIEW IF EXISTS unassigned_students CASCADE;
DROP VIEW IF EXISTS active_students CASCADE;
DROP VIEW IF EXISTS inactive_students CASCADE;
DROP VIEW IF EXISTS student_enrollment_details CASCADE;
DROP VIEW IF EXISTS unassigned_active_students CASCADE;
DROP VIEW IF EXISTS teacher_profile_summary CASCADE;
DROP VIEW IF EXISTS student_class_details CASCADE;

-- Also drop any other views that might exist from previous runs
DROP VIEW IF EXISTS student_details CASCADE;
DROP VIEW IF EXISTS class_enrollment_view CASCADE;
DROP VIEW IF EXISTS academic_summary CASCADE;

-- 2. Add enrollment status field to student_profiles table
...
-- 3. Update teacher_profiles to include profile photo (if not already added)
ALTER TABLE teacher_profiles 
ADD COLUMN IF NOT EXISTS profile_photo TEXT;

COMMENT ON COLUMN teacher_profiles.profile_photo IS 'Base64 encoded profile photo image data';

-- 4. Now safely alter academic_year field (after dropping ALL dependent views)
-- First check if the column exists and what type it is
DO $$
BEGIN
    -- Try to alter the column type
    BEGIN
        ALTER TABLE student_profiles ALTER COLUMN academic_year TYPE TEXT;
    EXCEPTION 
        WHEN OTHERS THEN
            -- If it fails, the column might not exist or already be TEXT
            -- Try to add it if it doesn't exist
            BEGIN
                ALTER TABLE student_profiles ADD COLUMN academic_year TEXT;
            EXCEPTION 
...
-- 5. Make sure section and grade_level are properly indexed
...
-- 6. Update classes table to ensure proper constraints
ALTER TABLE classes 
...
-- 7. Add updated_at column to key tables if not exists
ALTER TABLE student_profiles 
...
-- 10. Create functions for data integrity
...
-- 11. NOW recreate the enhanced views (after ALL column alterations are complete)
...
-- 12. Create comprehensive student enrollment view
...
-- 13. Recreate the student_class_details view (the one causing the error)
...
-- 14. Create unassigned active students view (updated)
...
-- 15. Create teacher profile summary view
...
GROUP BY tp.id, tp.first_name, tp.last_name, tp.email, tp.title, 
...
-- 16. Grant access to views for authenticated users
...
-- 17. Optional: Data migration for existing records
...
-- 
-- All views have been safely recreated!
-- ===============================================